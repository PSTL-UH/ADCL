...
{ int i;
  ...
  { int min1 = tree[i].left;
    ...
    //fprintf (file, "%s\t%s\t%s\t%d\t%d\t%d", nodeID[i], ID1, ID2, counts1+counts2, counts1, counts2);
    fprintf (file, "%s\t%s\t%d\t%s\t%d\t%d\t", nodeID[i], ID1, counts1, ID2, counts2, counts1+counts2);
    fprintf (file, "%f\n", 1.0-tree[i].distance);
