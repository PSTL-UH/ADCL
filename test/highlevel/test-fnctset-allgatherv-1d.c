...
/**********************************************************************/
{
    ...
    sdata = (double *) calloc(stdim, sizeof(double));
    rdata = (double *) calloc(rtdim, sizeof(double));

    err = ADCL_Allgatherv_init ( sdata, stdim, MPI_DOUBLE, rdata, rcnts, displ, MPI_DOUBLE, MPI_COMM_WORLD, &request);
    if ( ADCL_SUCCESS != err) goto exit;
