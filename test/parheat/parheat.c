...
/* MIR */
  ...
  if( myid == 0 )
  {
    MPE_Describe_state( 1, 2, "Bcast", "orange:vlines3" );
    MPE_Describe_state( 3, 4, "Isend", "red:vlines2" );
    MPE_Describe_state( 5, 6, "Irecv", "magenta:vlines1" );
    MPE_Describe_state( 7, 8, "Compt", "green:light_grey" );
/*
    MPE_Describe_state( 9, 10, "AllRd", "blue:grey3" );
*/
  }
#endif
  ...
  position = 0;
  numvars = 17;
  if( ierr=MPI_Pack_size( numvars, MPI_DOUBLE, newcomm, &buffsize ) != 0 )
  {
    printf( "MPI_Pack_size failed with code %d.\n", ierr );
    exit( ierr );
  }

  buffer = (void *)malloc( (unsigned)buffsize );
  if( myid==0 )
  {
...
/* MIR */
    ...
    MPI_Pack( &c_fact, 1, MPI_DOUBLE, buffer, \
              buffsize, &position, newcomm );
    MPI_Pack( &delta_t, 1, MPI_DOUBLE, buffer, \
              buffsize, &position, newcomm );
    MPI_Pack( &delta_x, 1, MPI_DOUBLE, buffer, \
              buffsize, &position, newcomm );
    MPI_Pack( grid, 3, MPI_INT, buffer, \
              buffsize, &position, newcomm );
    MPI_Pack( &mem_fac, 1, MPI_INT, buffer, \
              buffsize, &position, newcomm );
    MPI_Pack( &msg_fac, 1, MPI_INT, buffer, \
              buffsize, &position, newcomm );
    MPI_Pack( &cpt_fac, 1, MPI_INT, buffer, \
              buffsize, &position, newcomm );
    MPI_Pack( &accuracy, 1, MPI_DOUBLE, buffer, \
              buffsize, &position, newcomm );
    MPI_Pack( &tstep_fac, 1, MPI_DOUBLE, buffer, \
              buffsize, &position, newcomm );
    MPI_Pack( min, 3, MPI_DOUBLE, buffer, \
              buffsize, &position, newcomm );
    MPI_Pack( max, 3, MPI_DOUBLE, buffer, \
              buffsize, &position, newcomm );
    ...
    MPI_Bcast( buffer, position, MPI_PACKED, 0, newcomm );
  ...
  if( myid != 0 )
  {
    ...
    MPI_Bcast( buffer, buffsize, MPI_PACKED, 0, newcomm );
    ...
    MPI_Unpack( buffer, buffsize, &position, \
                &c_fact, 1, MPI_DOUBLE, newcomm );
    MPI_Unpack( buffer, buffsize, &position, \
                &delta_t, 1, MPI_DOUBLE, newcomm );
    MPI_Unpack( buffer, buffsize, &position, \
                &delta_x, 1, MPI_DOUBLE, newcomm );
    MPI_Unpack( buffer, buffsize, &position, \
                grid, 3, MPI_INT, newcomm );
    MPI_Unpack( buffer, buffsize, &position, \
                &mem_fac, 1, MPI_INT, newcomm );
    MPI_Unpack( buffer, buffsize, &position, \
                &msg_fac, 1, MPI_INT, newcomm );
    MPI_Unpack( buffer, buffsize, &position, \
                &cpt_fac, 1, MPI_INT, newcomm );
    MPI_Unpack( buffer, buffsize, &position, \
                &accuracy, 1, MPI_DOUBLE, newcomm );
    MPI_Unpack( buffer, buffsize, &position, \
                &tstep_fac, 1, MPI_DOUBLE, newcomm );
    MPI_Unpack( buffer, buffsize, &position, \
                min, 3, MPI_DOUBLE, newcomm );
    MPI_Unpack( buffer, buffsize, &position, \
                max, 3, MPI_DOUBLE, newcomm );
  }
    ...
    for ( i=0;i<ndim; i++)
	  {
	    ...
	    for ( j=0; j<ndim;j++)
		 ...
		 if (ncoords[i] == -1)
		   ...
		   {
		   MPI_Cart_rank(newcomm, ncoords,&neighbor[i]);
		   }
       }
    for ( i=0;i<ndim; i++)
	  {
	    ...
	    for ( j=0; j<ndim;j++)
		 ...
		 if (ncoords[i] == dimlist[i]+1)
		   {
			 neighbor[i+3] = MPI_PROC_NULL;
           }
		   else
		   {
		   MPI_Cart_rank(newcomm, ncoords,&neighbor[i+3]);
		   }
       }
		   printf("%d  coords %d %d %d  \n", myid ,coords[1],coords[1],coords[2]);
		   printf("%d  neighbors %d %d %d  \n", myid ,neighbor[1],neighbor[1],neighbor[2]);
...
/*
    ...
    { 
      ...
      if( neighbor[i] != -1 )
      {
        MPI_Cart_coords( newcomm, neighbor[i], ndim, coords );
        for( j=0 ; j<ndim ; j++ )
          printf( "coords[%d]=%d   ", j, coords[j] );
        printf( "\n" );
      }
    }
*/
...
/*
 * This Barrier was necessary for debugging
 * The program itself can do without
 */
  MPI_Barrier( newcomm );
