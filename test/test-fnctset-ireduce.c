...
int main (int argc, char *argv[])
{
  ...
  */
  ...
  sdata = (int*) calloc(dims*size, sizeof(int));
  rdata = (int*) calloc(dims*size, sizeof(int));

  //////////////////////////

  ADCL_Ireduce_init(sdata, rdata, dims, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD, &request);
  ...
  for ( i = 0; i<maxit ; i++ ) {  
    ...
    //NBC_Ireduce(&data, &data, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD, &handle);
