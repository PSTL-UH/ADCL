...
int ADCL_subarray_ext_init ( int ntopodim, int nvecdims, int *vecdims,
             ...
             MPI_Datatype **senddats, MPI_Datatype **recvdats)
{
    ...
    sdats = ( MPI_Datatype *) malloc ( nneigh * 2 * sizeof(MPI_Datatype));
    rdats = ( MPI_Datatype *) malloc ( nneigh * 2 * sizeof(MPI_Datatype));
    if ( NULL == sdats || NULL == rdats  ) {
        ret = ADCL_NO_MEMORY;
        goto exit;
    }
